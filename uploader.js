...
BPTool.Uploader = typeof BPTool.Uploader != "undefined" && BPTool.Uploader ? BPTool.Uploader : function() {
    ...
    return {
        create: function(containingDiv, initParams) {
            ...
            if (ConfigParams.archiveFormat != "") {
                ServiceList.push({service: "Archiver", version: "1", minversion:"1.1.0"});
            }
            ...
            return function() {
                ...
                function fireActionListener(file, action, success, value, error, verboseError)
                {
                ...
                function addFile(file, relativeName) {
                    ...
                    if ((!MimeTypes || matchMime(MimeTypes, file.mimeType))
                        && !matchMime({"application/x-folder": 1}, file.mimeType)) {
                        fireActionListener(file, ACTION.ADD);
                        file.bytesSent = 0;
                        droppedFiles.push({file: file, relativeName: relativeName});
                ...
                function addFiles(children) {
                    for (var i = 0; i < children.length; i++) {
                        var kid = children[i];
                        var file = kid.handle;
                        if (matchMime({"application/x-folder": 1}, file.mimeType)) {
                            addFiles(kid.children);
                        } else {
                            addFile(file, kid.relativeName);
                        }
                    }
                }
                ...
                function onFileSelect(files) {
                    ...
                    if (ConfigParams.mimeTypes) {
                        dirArgs.mimetypes = ConfigParams.mimeTypes;
                    }
                    if (ConfigParams.maxFiles) {
                        dirArgs.limit = ConfigParams.maxFiles;
                    }
                    BrowserPlus.Directory.recursiveListWithStructure(dirArgs, function(res) {
                        if (res.success) {
                            addFiles(res.value.files);
                        }
                        renderCoreUI();
                    })
                }
                        ...
                        if (el.className == (dropZoneId + "_remove")) {
                            ...
                            if (a[0]) {
                                ...
                                if (selectedFileId) {
                                    ...
                                    fireActionListener(droppedFiles[selectedFileId.match(/[0-9]+$/)[0]].file,
                                                       ACTION.UNSELECT);
                                    selectedFileId = null;
                                }

                                // remove the file from the list
                                fireActionListener(droppedFiles[a[0]].file, ACTION.REMOVE);
                    ...
                    if (ConfigParams.uploadUrl) {
                        ...
                        if (ConfigParams.archiveFormat != "") {
                            ...
                            for (var i = 0; i < droppedFiles.length; i++) {
                                theFiles.push(droppedFiles[i].file);
                            }
                            BrowserPlus.Archiver.archive({files: theFiles,
                                                         format: ConfigParams.archiveFormat,
                                                         recurse: false},
                                function (res) {
                                    if (res.success) {
                ...
                function renderCoreUI() {
                    ...
                    for (i = 0; i < droppedFiles.length; i++) {
                        var file = droppedFiles[i].file;
                        var relName = droppedFiles[i].relativeName;
                        sizeInBytes = "";
                        if (file.size) {
                            sizeInBytes = bytesToString(file.size);
                            totalInBytes += file.size;
                        }
